/**
 * Apply database/schema.sql to PostgreSQL using DATABASE_URL from .env.local
...
const psql = findPsql();
if (!psql) {
  console.error("psql not found. Install PostgreSQL or set PSQL_PATH.");
  process.exit(1);
}
...
const result = spawnSync(
  psql,
  ["-U", user, "-h", host, "-p", port, "-d", database, "-v", "ON_ERROR_STOP=1", "-f", schemaPath],
  {
    encoding: "utf8",

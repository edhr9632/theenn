      ...
      <main className="events-page">
        <section className="events-hero" aria-labelledby="events-hero-heading">
          <div className="container">
            ...
            <h1 id="events-hero-heading" className="events-hero-title serif-headline mb-3 mb-lg-4">Convene. Discuss. Build.</h1>
            <p className="events-hero-deck mb-0">Summits, workshops, and live conversations that connect policymakers, educators, and industry leaders.</p>
          </div>
        </section>
        <div className="events-filter-bar">
          <div className="container">
            <nav className="d-flex flex-wrap align-items-center gap-2 py-3" aria-label="Event categories">
              <Link className="events-filter-pill is-active" href="/events" aria-current="page">Featured events</Link>
              <Link className="events-filter-pill" href="/events/speakers">Speakers</Link>
              <Link className="events-filter-pill" href="/events/sponsors">Sponsors</Link>
            </nav>
          </div>
        </div>
                ...
                <article className="event-card h-100 bg-white rounded-3 shadow-sm overflow-hidden d-flex flex-column">
                  ...
                  <div className="event-card-body d-flex flex-column flex-grow-1 p-3 p-md-4">
                    <h2 className="event-card-title serif-headline h5 mb-2">
                      <a href="#" className="event-card-title-link text-decoration-none">{event.title}</a>
                    </h2>
                    <p className="event-card-desc small text-secondary mb-3">{event.excerpt}</p>
                    <div className="event-card-meta small mt-auto">
                      <div className="event-meta-row"><span>{event.date}</span></div>
                      <div className="event-meta-row"><span>{event.location}</span></div>
                    </div>
                  </div>
                </article>
              </div>
